...
/*
  ...
  - 문자열에 덧붙이고자 하는 문자를 왼쪽 또는 오른쪽에 덧붙여서 최종적으
    반환할 문자의 길이만큼 문자열 반환
*/
...
SELECT timediff('2024-01-01 00:00:00', now()); -- 123:41:04
...
/*
  ...
  ADDTIME(날짜, 시간정보)
  -특정 날짜에 입력받은 정보만큼 더한 날씨를 반환
  
  SUBDATE(날짜, INTERVAL 숫자 날짜단위
  SUBTIME(날짜, 시간정보)
  - 특정 날짜에 입력받은 정보만큼 뺸 날짜를 반환
*/
SELECT now(), adddate(now(), interval 15 year);
...
SELECT emp_name, hire_date, date_format(hire_date, '%Y년.%m월.%d일')
FROM employee;
...
SELECT emp_name, job_code, format(salary, 0) 급여,
...
SELECT format(sum(salary * 12), 0) "연봉합계"   
FROM employee
WHERE dept_code = 'D5';
...
/*
  AVG(숫자)
  - 해당 컬럼 값들의 평균값을 반화
  - 해당 그룹 합수는 NILL 값을 자동으로 제외
    AVG 함수를 사용할 때는 COALESCE 또는 IFNULL 함수와 함께
    사용하는 것을 권장
*/
-- 전체 사원의 평균 급여, 보너스율 조회
SELECT avg(salary), avg(ifnull(salary, 0)),
...
*/
-- 가장 작은 값에 해당하는 사원명, 급여, 입사일(hire_date)
...
FROM employee;
-- 현대 사원들이 분포되어 있는 직급 수 조회 (job_code)
