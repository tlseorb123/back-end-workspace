...
*/
-- 부서별 평균 급여가 300먼원 이상인 부서의 평균 급여 조회
SELECT dept_code, format(avg(ifnull(salary, 0)), 0) "평균 급여"
FROM employee
GROUP BY dept_code
...
/*
  ...
  저희 문제에도 안 나옴!!!! NENER!!!! 몰라도 됨!!!!!
*/
-- 직급별 급여 합 조회
SELECT job_code, job_code, sum(salary), grouping(job_code) 
FROM employee
...
/*
  ...
  MINUS (차집합) : 선행 쿼리문의 결과값에서 후행 쿼리문의 결과값을 뺀 나머지 결과에만 추출(MySQL X)
