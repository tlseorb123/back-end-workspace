...
ALTER TABLE dept_copy MODIFY drpt_title VARCHAR(5);
...
MODIFY lname VARCHAR(20) DEFAULT '미국'; -- DEFAULT은 NULL인 경우를 미국으로 표시

/*
   1-3. 컬럼 삭제 
   
   ALTER TABLE 테이블명 DOPR COLUMN 컬럼명; 
   
   - 데이터 값이 기록되어 있어도 같이 삭제된다. ( 삭제된 컬럼 복구는 붕가능)
...
ALTER TABLE member DROP FOREIGN KEY member_grade_ib_fk;

/*
   1-4. 컬럼명 변경(RENAME COLUMN) 
   ALTER TABLE 테이블명 RENAME COLUMN 기존컬럼명 TO 변경할컬럼명;
*/
-- dept_copy 테이블에서 lnama 컬럼명을 location_name으로 변경
ALTER TABLE dept_copr RENAME COLUMN lname TO locaTION_name;

/*
    2. 제약조건 추가 / 삭제 
    2-1. 제약조건 추가(ADD)
   -- ADD [CONSTRAINT 제약조건명] PRIMARY KET(컬럼명);
   -- ADD [CONSTRAINT 제약조건명] FOREIGN KET(컬럼명) REFERENCES 테이블명(컬럼명)
   -- ADD [CONSTRAINT 제약조건명] UNIQUE(컬럼명);
   -- ADD [CONSTRAINT 제약조건명] CHECK(컬럼에 대한 조건);
   -- NOT NULL : MODIFY 컬럼명 [CONSTEAINT 제약조건명] NOT NULL;
   */
...
-- 어딘가에 참조되고 있는 부모테이블은 함부로 삭제가 안됨!
