...
*/
-- '한국'에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가 조회 
SELECT * FROM employee; -- emp_id, emp_name, salart, dept_code
SELECT * FROM department; -- dept_id, dept_title, location_id
SELECT * FROM location; -- local_code, national_code
SELECT * FROM national; -- national_code, nation_name
SELECT * FROM job;
...
-- '러시아'에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가 조회
...
-- '일본'에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가 조회
...
-- DELETE (에러) : 삭제가 되는 조건 - 자식 데이터가 존재하지 않을 결우
DELETE FROM vw_job
WHERE job_name = '알바';
 ...
 -- 사원, 사원명, 급여, 연봉(salary * 12)을 조회한 SELECT 문으로 vw_emp_sal 뷰 정의 
...
-- 부서별 급여의 함계, 평균을 조회한 SELECT 문을 vw_groupdept 뷰를 정의
CREATE OR REPLACE VIEW vw_groupdept 
AS SELECT 
     dept_code,  
	 format (sum(salary)) "합계", 
...
CREATE OR REPLACE VIEW vw_joindept
AS SELECT emp_id, emp_name, dept_title
FROM employee 
JOIN department ON (dept_code = dept_id);

SELECT * FROM vw_joindept;

-- INSERT (에러) 
INSERT INTO vw_joindept 
VALUES(300, '조세오', '총무부');
...
-- 급여가 3000000원 이상인 사원들만 조회한 SELECT 문은 vw_emp 뷰 정의
...
-- 200번 사원의 급여를 200만원으로 변경 (뷰를 이용해서)
UPDATE vw_emp 	
SET salary = 2000000
WHERE emp_id = 200; 
