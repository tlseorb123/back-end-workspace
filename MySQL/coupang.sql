...
CREATE TABLE  enterprise(
          ente_id VARCHAR(50) PRIMARY KEY, -- 엄체 아이디 
...
CREATE TABLE  pick(
          ...
          id VARCHAR(50), -- 아이디 
          prod_code INT, -- 상품 코드 
          pick_date DATE DEFAULT(current_date) -- 찜
...
CREATE TABLE  purchase(
          ...
          pur_status VARCHAR(10) CHECK (pur_status IN ('구매대기중', '구매중', '배송대기중', '배송중'))
									DEFAULT '구매대기중'  -- 구매 상태
); -- 구매 내역 
CREATE TABLE  delivery(
          deli_code INT AUTO_INCREMENT PRIMARY KEY, -- 배송 코드
          pur_code INT, -- 구매 코드 
          deli_date DATE DEFAULT (current_date), -- 배송사 
          deli_cost INT, -- 배송비 
...
CREATE TABLE  swap(
          ...
          price INT, -- 비용 
          swap_status CHAR(2) CHECK(swap_status IN ('교환', '반품'))
); -- 교환/반품 
CREATE TABLE  coupon(
          coupon_code INT AUTO_INCREMENT PRIMARY KEY, -- 쿠폰 코드 
          coupon_neme VARCHAR(30), -- 쿠폰명 
...
ADD FOREIGN KEY (parent_code) REFERENCES (cate_code);
...
ADD FOREIGN KEY (pur_code) REFERENCES product(prod_code); 
