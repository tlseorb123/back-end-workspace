...
public class A_ArrayList {
	...
	public void method2() {
		...
		list.add(2, new Person("신대규", "천호동", 11));
		
		// 3. set : 해당 인덱스의 값을 변경
		list.set(3, new Person("오윤영", "삼성동", 57));
	    list.set(2, new Person("김길영", "천호동", 11));
		...
		int sum = 0;
		/*
		 * for(int i = 0; 1< list.size(); i++) { sum += list.get(i).getAge(); }
		 */
		for (Person p : list) {
			sum += p.getAge();
		}
	...
	public void method3() {
		...
		// 11. 오름차순 정렬 : Comparble 인터페이스를 구현하고 있는 요소를 가지고 비교한 값들 반환하며 정렬  
	//	Collections.sort(list);
