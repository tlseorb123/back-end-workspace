...
//		f.method1();
	...
	public void fileSave() {
		...
		try (FileWriter fw = new FileWriter(fileName, true)) {
			// true 를 쓰면 자료가 이어서 써지고 지우면 덧붙였던 글이 사라진다.
			fw.write("IO 재밌나요?\n");
			fw.write("아닌가요 ㅠㅠ?\n");
		 ...
		 * } catch (IOException e) {
		 * 
		 * e.printStackTrace(); }finally { // 자원반납 try { fw.close(); } catch
	...
	// 파일에 있는 내용을 읽어서 한 줄 단위로 콘솔에 줄력하는 로직
	// 보조 스트릭 BufferedReader : 버퍼(char[])에 데이터가 쌓이기를 기다렸다가 꽉 차게 될 시 한꺼번에 보낸다.
	public void method1() {
	...
	public void method2() {
		...
		// 뒤에서부터 순서대로 기반스크림이다. 지금은 1줄로 쓰는 법
